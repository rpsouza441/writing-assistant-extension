...
export class InputAdapter extends BaseTextAdapter<HTMLInputElement> {
  readonly source = "input" as const;
  ...
  canAppend(): boolean {
    return isSupportedInput(this.element) && !this.element.disabled && !this.element.readOnly;
  }
  ...
  getSelection(): AdapterSelection | null {
    if (!isSupportedInput(this.element) || this.element.disabled || this.element.readOnly) {
      return null;
    }
  ...
  appendText(text: string): InsertionResult {
    ...
    const nextText = `${this.element.value}${appendSeparator(this.element.value, false)}${text}`;
...
export function isSupportedInput(element: HTMLInputElement): boolean {
  return SUPPORTED_INPUT_TYPES.has(element.getAttribute("type")?.toLowerCase() ?? element.type);
}
