...
export class RichTextFallbackAdapter extends BaseTextAdapter<HTMLElement> {
  readonly source = "rich-text-fallback" as const;
  ...
  getSelection(): AdapterSelection | null {
    ...
    return this.selectionFrom(text, safeRangeRect(range, this.element), true, true);
  }

  appendText(text: string): InsertionResult {
    this.focus();

    const separator = appendSeparator(this.getText(), true);
